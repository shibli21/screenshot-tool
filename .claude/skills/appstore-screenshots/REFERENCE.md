...
Fill `{N}`, `{app}`, `{layout}`, and reference the original screenshot's style.
...
of the attached original screenshot. Each of the {N} panels is a polished marketing frame:
a clean device UI mockup plus a short bold headline caption.
...
- independent: each panel is self-contained with its own background and headline.

Constraints: no watermarks, no Apple logos, legible headline text, high resolution,
consistent margins so the strip can be cut into {N} equal vertical slices.
```

## Choosing layout (step 2)
...
Key features (one per screenshot): 1) … 2) … 3) …
Brand colors: #...
```
